...
clk_hw_requests_find_hw_request_by_clk_core(const struct clk_hw_requests *hw_reqs,
					    const struct clk_core *core)
{
	struct clk_hw_request *hw_req;
...
static struct clk_hw_request *
clk_hw_request_build_one(const struct clk_request *req,
			 struct clk_hw_requests *hw_reqs,
			 const struct clk_core *core)
{
	struct clk_hw_request *hw_req;
	...
	hlist_for_each_entry(child, &core->children, child_node) {
		...
		child_hw_req = clk_hw_requests_find_request_or_alloc(req, hw_reqs, child);
				      ...
				      const struct clk_core *core)
{
	...
	pr_crit("%s +%d\n", __func__, __LINE__);
	return clk_hw_request_build_one(req, hw_reqs, core);
}
...
static bool clk_core_is_in_request(const struct clk_core *core, const struct clk_request *req)
{
	...
	if (req->hw_reqs) {
		...
		hw_req = clk_hw_requests_find_hw_request_by_clk_core(req->hw_reqs, core);
		if (hw_req)
			return true;
	}
...
static bool clk_hw_requests_is_clk_core_top(const struct clk_hw_requests *hw_reqs,
					    const struct clk_core *core)
{
	struct clk_hw_request *parent_hw_req;
	struct clk_core *parent = core->parent;

	if (!parent)
		return true;

	parent_hw_req = clk_hw_requests_find_hw_request_by_clk_core(hw_reqs, parent);
	if (!parent_hw_req)
		return true;
...
{
	if (req->hw_reqs)
		return list_count_nodes(&req->hw_reqs->requests);
				       ...
				       struct clk_hw_request *hw_req)
{
	...
	if (hw_req->desired.parent == hw_req->requested.parent)
		return 0;

	if (hw_req->desired.parent == core->parent)
		return 0;
...
static int
clk_request_check_handle_parent_rate_change(struct clk_request *req,
					    struct clk_hw_requests *hw_reqs,
					    struct clk_hw_request *hw_req)
{
	const struct clk_core *core = hw_req->core;
	...
	parent_req = clk_hw_requests_find_hw_request_by_clk_core(hw_reqs, parent);
	if (parent_req) {
		unsigned long long parent_rate;

		if (parent_req->requested.rate_set)
			parent_rate = parent_req->requested.rate;
		else
			parent_rate = clk_core_get_rate_nolock(parent);

		if (hw_req->desired.parent_rate == parent_rate)
			return 0;

		pr_info("req-%lld: %s: Parent rate changed, updating parent rate request.\n",
			req->id, core->name);
	...
	pr_info("req-%lld: %s: Parent rate changed, adding clock to request and restarting.\n",
		req->id, core->name);
				      ...
				      unsigned int try)
{
	...
	ret = clk_request_check_handle_parent_rate_change(req, hw_reqs, hw_req);
	if (ret) {
		if (ret == -EAGAIN)
			restart = true;
		else
				    ...
				    unsigned int try)
{
	...
	hlist_for_each_entry(child, &core->children, child_node) {
		...
		child_hw_req = clk_hw_requests_find_hw_request_by_clk_core(hw_reqs, child);
		if (!child_hw_req)
			continue;
				 ...
				 unsigned int try)
{
	...
	list_for_each_entry(hw_req, &hw_reqs->requests, node) {
		...
		if (!clk_hw_requests_is_clk_core_top(hw_reqs, core)) {
