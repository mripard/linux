...
static const struct reg_value pwet[] = {
	{0x3103, 0x11, 0, 0},//
	{0x3008, 0x42, 0, 0},//
	{0x3103, 0x03, 0, 0},//
	{0x3017, 0xff, 0, 0},//
...
//{0x3821, 0x07, 0, 0},//
	...
	{0x302c, 0xc2, 0, 0},//bit[7:6]: output drive capability
...
static const struct reg_value pwet_vga[] = {
/*
	{0x3035, 0x11, 0, 0},
	{0x3a00, 0x78, 0, 0},
...
static int ov5640_pwet(struct ov5640_dev *sensor, u16 reg, u8 val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg;
	u8 buf[3];
...
//	printk("ov5640: reg=0x%04x val=0x%02x\n", reg, val);
	ret = i2c_transfer(client->adapter, &msg, 1);
	if (ret < 0) {
...
//	printk("ov5640: b: reg=0x%04x val=0x%02x\n", reg, val);
	ret = i2c_transfer(client->adapter, &msg, 1);
	if (ret < 0) {
		dev_err(&client->dev, "%s: error: reg=%x, val=%x\n",
			__func__, reg, val);
...
static int ov5640_load(struct ov5640_dev *sensor, const struct reg_value *regs,
		       size_t len)
{
	unsigned int i;
	int ret = 0;
	...
	for (i = 0; i < len; ++i, ++regs) {
		u16 reg_addr = regs->reg_addr;
		u8 val = regs->val;

		ret = ov5640_pwet(sensor, reg_addr, val);
		if (ret)
			break;
	}
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	ov5640_load(sensor, pwet, ARRAY_SIZE(pwet));
	ov5640_load(sensor, pwet_vga, ARRAY_SIZE(pwet_vga));
