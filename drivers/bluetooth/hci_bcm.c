...
static int bcm_common_probe(struct bcm_device *dev)
{
	 ...
	 *
	 * If it hasn't been filled by the calling function, fall back
	 * on the host wake-up GPIO, that is common to all probing
	 * mechanisms.
	 */
	if (dev->irq <= 0) {
...
static int bcm_acpi_probe(struct bcm_device *dev)
{
	...
	ret = acpi_dev_add_driver_gpios(ACPI_COMPANION(dev->dev),
					gpio_mapping);
	if (ret)
		return ret;
